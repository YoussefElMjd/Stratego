...
void Board::fillArray(int power, PLAYER player, int row, int col)
{
    switch (power)
    {
        case 10 : battleField[row][col] = GameElement(MARECHAL, power,
                                              player);
            break;
        ...
        case 6 : battleField[row][col] = GameElement(COMMANDANT, power,
                                             player);
            break;

        case 5 : battleField[row][col] = GameElement(LIEUTENANT, power,
                                             player);
            break;
        ...
        case 2 : battleField[row][col] = GameElement(ECLAIREUR, power,
                                             player);
            break;
...
void Board::initializeBattleField(Position p, PLAYER player,
                                  GameElement s)
{
...
int Board::endGame()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        for (int j = 0; j < 10; j++)
        {
            ...
            {
                if (battleField[i][j].getRole() != BOMBE &&
                        battleField[i][j].getRole() != DRAPEAU)
                {
                    cptRed++;
                }
            }
...
bool Board::moveEclaireur(Position & p, Position & toGo, Position dir,
                          int distance)
{
    int i = 0;
    while (canBePlace(toGo) && i < distance)
    {
        battleField[toGo.getX()][toGo.getY()] =
            battleField[p.getX()][p.getY()];
        removeFromBattle(p);
        i++;
        p = toGo;
        toGo += dir;
    ...
    {
        if (i < distance && !getSoldier(toGo).isDecor() &&
                getSoldier(toGo).getColor() != getSoldier(p).getColor())
        {
            return true;
        }
    }
...
std::optional<GameElement> Board::move(Position p, DIRECTION dir,
                                       int distance)
{
    if (!getSoldier(p).isDecor() && getSoldier(p).getRole() != DRAPEAU &&
            getSoldier(p).getRole() != BOMBE)
    {
        Position direction;
        switch (dir)
        {
...
{
    if (pos.getY() > 9 || pos.getX() < 0 || pos.getX() > 9 ||
            pos.getY() < 0 )
        return false;
...
GameElement Board::attack(Position p, Position toAttack)
{
    ...
    {
        battleField[toAttack.getX()][toAttack.getY()] =
            battleField[p.getX()][p.getY()];
        removeFromBattle(p);
    }

...
#include "gameElement.h"    // rnvs : include / déclaration anticipée
...
void Game::initializeBattleField(const Position & p, PLAYER player,
                                 const GameElement & s)
{
    if (this->state != PLACEMENT)
    {
...
void Game::makeMove(optional<GameElement> & saveSoldier, Position p,
...
//optional<GameElement> Game::move(Position & p, DIRECTION & d,
//                                 int distance)    // rnvs : comm
optional<GameElement> Game::move(const Position & p, DIRECTION & d,
                                 int distance)  // rnvs : ajout
{
    if (this->state != MOVE)
    {
        invalid_argument("Le status n'est pas à MOVE");
...
bool Game::goodFile(vector<string> const & pieces)
{
    ...
    return numberPiece.size() == realNumberPiece.size() &&
           equal(numberPiece.begin(), numberPiece.end(),
                 realNumberPiece.begin());

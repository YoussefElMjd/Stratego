...
#include <iomanip>          // rnvs : include / déclaration anticipée
#include "model/position.h" // rnvs : include / déclaration anticipée
#include "model/gameElement.h"  // rnvs : include / déclaration anticipée
#include <iostream>     // rnvs : include / déclaration anticipée
#include "model/game.h" // rnvs : include / déclaration anticipée

using namespace std;
// using namespace nvs;     // rnvs : include / déclaration anticipée
    ...
    for (size_t i = 0; i < 10 ; i++)
    {
        ...
        for (size_t j = 0; j < 10; j++)
        {
            ...
            else
            {
                ...
                {
                    cout << std::setw(3) << "B" << model->getSoldier(Position(i,
                            j)).value();
                }
                else if (model->getSoldier(Position(i, j)).value().getColor() == RED)
                {
                    cout << std::setw(3) << "R" << model->getSoldier(Position(i,
                            j)).value();
                }
                else
                {
...
void View::displayAttacker(std::optional<GameElement> soldier,
                           Position pos)
{
    if (soldier != nullopt)
    {
        ...
        if (model->getCurrentPlayer() == BLUE)
        {
            if (role == "BOMBE")
            {
                cout << "!!!! BOUMMMMMMMMMMMMM VOTRE SOLDAT EST MORT D'UNE BOMBE !!!!"
                     << endl;
            }
            else
            {
                cout << "!!!! LE JOUEUR ROUGE VOUS A ATTAQUÉ AVEC " << role <<
                     " PUISSANCE" << soldier.value()
                     << " DEPUIS LA POSITION " << y << pos.getX() + 1 << "!!!" <<
                     std::endl;
            }
        }
        else
        {
            ...
            {
                cout << "!!!! LE JOUEUR BLEU VOUS A ATTAQUÉ AVEC " << role <<
                     " PUISSANCE" << soldier.value()
                     << " DEPUIS LA POSITION " << y << pos.getX() + 1 << "!!!" <<
                     std::endl;
            }

...
private:
    ...
    */
    void makeMove(std::optional<GameElement> & saveSoldier,Position p, Position direction, DIRECTION d,int distance);
    /**
...
public:
    ...
    */
    void initializeBattleField(const Position & p, PLAYER player,const GameElement & s);
    /**
    ...
    * @return
    */
    std::optional<GameElement> move(Position & p, DIRECTION & d, int distance = 1);
    /**
    ...
    */
    PLAYER getWinnerPlayer() const; // c'est la même chose que getCurrentPlayer() parce que le gagnant est d'office égale au joueur courant
    /**
    * @brief getCurrentPlayer allows to give the current player
    * @return a color of PLAYER enumeration
    */
    PLAYER getCurrentPlayer() const; // c'est la même chose que getWinnerPlayer():
    /**

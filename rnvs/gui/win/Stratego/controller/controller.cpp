...
void Controller::fillVectorSoldier(vector<GameElement> & soldier, PLAYER color){
    soldier = {
...
void Controller::initialiseGamePlacement(PLAYER currentPlayer){
    ...
    for (size_t i = 0 ; i < soldierRed.size(); i++)
    {
        if(currentPlayer == RED){
            ...
            cout<< "Vous devez placer votre " << pieceName.at(soldierRed.at(i).getRole().value()) << " entrez une position" << endl;
        } else {
            cout<<"Placer vos soldats joueur BLEU"<< endl;
            cout<< "Vous devez placer votre " << pieceName.at(soldierBlue.at(i).getRole().value()) << " entrez une position" << endl;
        }
        Position p;
        do {
            cout << "Attention vous ne pouvez placer que dans votre camps" << endl;
...
int Controller::askDistance(){
    ...
    cout << "Veuillez entrez la distance que l'eclaireur doit parcourir, prenez garde si vous entrez une distance "
            "qui fera que l'eclaireur ce trouvera à l'extérieur celui-ci s'arrêtera juste avant " << endl;
    do{
        try{
...
bool Controller::conditionPosition(Position pos, Position toGo){
    ...
    if(col > 9 || row < 0 || row > 9 || col < 0 ){
        cout << "Player " << currentPlayer << " vous ne pouvez pas placer en dehors du champ de bataille" << endl;
        return false;
    }
    if(model->getSoldier(pos)->getRole() == DRAPEAU){
        cout << "Joueur " << currentPlayer << " vous ne pouvez pas deplacer "
                                              "votre drapeau choisissez de nouveau un soldat à bouger et une direction" << endl;
        return false;
    }

    if(model->getSoldier(pos)->getRole() == BOMBE){
        cout << "Joueur " << currentPlayer << " vous ne pouvez pas deplacer "
                                              "votre bombe choisisez de nouveau un soldat à bouger et une direction" << endl;
        return false;
    }
    if(model->getSoldier(toGo)->isWater()){
        cout << "Joueur " << currentPlayer << " vous ne pouvez pas deplacer sur de l'eau" << endl;
        return false;
    }
    if(model->getSoldier(toGo) != nullopt && !model->getSoldier(toGo)->isDecor() && model->getSoldier(toGo)->getColor() == model->getCurrentPlayer()){
        cout << "Joueur " << currentPlayer << " vous ne pouvez pas deplacer ici, il y a déjà l'un de vos pions" << endl;
        return false;
    }
    return true;
}
...
DIRECTION Controller::askDirection(){
    ...
    do {
        try {
            cout << "Entrez votre direction de déplacement, vous avez le choix entre" << endl;
    ...
    } while (dir != "GAUCHE" && dir != "DROITE" && dir != "HAUT" && dir != "BAS");
...
LEVEL Controller::askLevel(){
    ...
    do {
        try {
            cout << "Entrez le niveau de votre partie, vous avez le choix entre" << endl;
...
string Controller::askInitializeGame(){
    ...
    do {
        try {
            cout << "Entrez comment voulez-vous initialisé la partie vous avez deux choix" << endl;
